...
% --------- перестроение 1го автомобиля

% ------------ начальные параметры 1го автомобиля перед перестроением 
            
        [x_0_1, y_0_1, vx_0_1, vy_0_1, phi_0_1, omega_0_1] = ...
            get_cur_pos(x_y_vx_vy_phi_omega_grid_reaction_1(end, :));
    
        t_grid_change_and_left_1 = [t_grid_reaction(end), ...
                           t_grid_2(~t_reaction_mask)];
                       
        % numel(t_grid_change_and_left_1)
...
% ---------------- параметры для первого поворота при перестроении
        ...
        v_0_1 = norm([vx_0_1, vy_0_1]);
        turn_1_acc_rot_1 = 0;
        turn_1_R_1 = lane_width / (3 * (1 + cos(pi - rot_angle))); % по оси х переместиться на lane_width / 3, повернувшись на угол rot_angle;

        turn_1_t_1 = rot_angle * turn_1_R_1 / v_0_1;
    
        t_start = t_grid_change_and_left_1(1);
        t_grid_turn_1 = t_grid_change_and_left_1(...
                        t_grid_change_and_left_1 <= (t_start + turn_1_t_1));
    
        turn_1_motion_param_1 = struct(field_names.x, x_0_1, ...
                                   field_names.y, y_0_1, ...
                                   field_names.v, v_0_1, ...
                                   field_names.phi, phi_0_1, ...
                                   field_names.acc_rot, turn_1_acc_rot_1, ...
                                   field_names.R, turn_1_R_1, ...
                                   field_names.t, t_grid_turn_1);

% ---------------- параметры для прямолинейного движения при перестроении

        straight_motion_acc_tan_1 = 0;

        straight_motion_t_1 = lane_width / (3 * v_0_2 * sin(rot_angle));

        t_start = t_grid_turn_1(end);
        t_grid_straight_1 = t_grid_change_and_left_1(...
                            (t_start <= t_grid_change_and_left_1) & ...
                            (t_grid_change_and_left_1 <= (t_start + straight_motion_t_1)));
    
        straight_motion_param_1 = struct(field_names.acc_tan, straight_motion_acc_tan_1, ...
                                     field_names.t, t_grid_straight_1); 

% ---------------- параметры второго поворота при перестроении
                             
        turn_2_acc_rot_1 = 0;
        turn_2_R_1 = lane_width / (3 * (1 - cos(rot_angle)));

        turn_2_t_1 =  turn_2_R_1 * rot_angle / v_0_1;
    
        t_start = t_grid_straight_1(end);
        t_grid_turn_2 = t_grid_change_and_left_1(...
                        (t_start <= t_grid_change_and_left_1) & ...
                        (t_grid_change_and_left_1 <= (t_start + turn_2_t_1)));
    
        turn_2_motion_param_1 = struct(field_names.acc_rot, turn_2_acc_rot_1, ...
                                   field_names.R, turn_2_R_1, ...
                                   field_names.t, t_grid_turn_2);
    
% ---------------- обновляем параметры для отслеживания столкновения во время торможения 1го автомобиля
    
        t_grid_logical = ((t_grid_reaction(end) <= t_grid_2) & ...
                         (t_grid_2 <= t_grid_turn_2(end)));
    
        opts.intersection_opts = setfield(opts.intersection_opts, ...
                                          field_names.x, ...
                                          x_y_vx_vy_phi_omega_grid_2(t_grid_logical, 1));
                                      
        opts.intersection_opts = setfield(opts.intersection_opts, ...
                                          field_names.y, ...
                                          x_y_vx_vy_phi_omega_grid_2(t_grid_logical, 2));
        
        opts.intersection_opts = setfield(opts.intersection_opts, ...
                                          field_names.phi, ...
                                          x_y_vx_vy_phi_omega_grid_2(t_grid_logical, 5));
                                      
        opts.intersection_opts = setfield(opts.intersection_opts, ...
                                          field_names.t, ...
                                          t_grid_2(t_grid_logical));
                               
        
% ------------ моделирование перестроения 1го автомобиля (частичное, если произойдёт столкновение)

        disp('START 1st car lane change modelling');
      
        sol_change_1 = ...
            lane_change(turn_1_motion_param_1, ...
                    straight_motion_param_1, ...
                    turn_2_motion_param_1, ...
                    opts, ...
                    false);
        
        t_grid_change_1 = sol_change_1.x;
        x_y_vx_vy_phi_omega_grid_change_1 = (sol_change_1.y)';
        % x_y_vx_vy_phi_omega_grid_change_1(end, 5) = 0;
                
        disp('Временная сетка: ');
        disp(t_grid_change_1');
        disp('Параметры движения 1го автомобиля: ');
        disp(x_y_vx_vy_phi_omega_grid_change_1);
    
        disp('FINISH 1st car lane change modelling');
 
        if (~isfield(sol_change_1, 'xe')) || (numel(sol_change_1.xe) == 0) || (sol_change_1.ie(1) == 2) % 1й автомобиль перестроился и не врезался
            ...
            [x_0_1, y_0_1, vx_0_1, vy_0_1, phi_0_1, omega_0_1] = ...
                get_cur_pos(x_y_vx_vy_phi_omega_grid_change_1(end, :));

            t_grid_left_1_mask = (t_grid_2 >= t_grid_change_1(end));
...
v = VideoWriter('change_accident.avi'); % 'Uncompressed AVI'
v.FrameRate = 10;
open(v);
